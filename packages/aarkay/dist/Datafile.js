...
class Datafile {
    constructor(source, destination) {
        ...
        if (this.type === DatafileType_1.DatafileType.SINGLE) {
            ...
            this.contexts = yamlContents;
            this.filenames = this.contexts.map(contents => contents["_fn"] === undefined ? contents["name"] : contents["_fn"]);
            this.directories = this.contexts.map(contents => contents["_dn"] === undefined ? "" : contents["_dn"]);
        }
    }
    generatedFiles() {
        let generatedFiles = [];
